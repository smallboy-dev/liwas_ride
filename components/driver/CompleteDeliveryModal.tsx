...
import type { DriverOrder } from "@/hooks/useDriverData";
import { Badge } from "@/components/ui/badge";

interface CompleteDeliveryModalProps {
  order: DriverOrder | null;
  onClose: () => void;
  onComplete: (order: DriverOrder, signature: Blob) => Promise<void>;
}
...
function formatAddress(value: any): string {
  ...
  if (Array.isArray(value)) {
    return value
      .flatMap((entry) => formatAddress(entry).split(", "))
      .filter(Boolean)
      .join(", ");
  }
  ...
  if (typeof value === "object") {
    ...
    preferredKeys.forEach((key) => {
      if (Object.prototype.hasOwnProperty.call(value, key)) {
        seen.add(key);
        const segment = formatAddress(value[key]);
    ...
    return segments.length ? Array.from(new Set(segments)).join(", ") : "N/A";
  }

  return "N/A";
}

export function CompleteDeliveryModal({ order, onClose, onComplete }: CompleteDeliveryModalProps) {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (!hasSignature) {
      setError("Capture the customer's signature before completing the delivery.");
      return;
    }
    ...
    try {
      ...
      await onComplete(order, signatureBlob);
      onClose();
    } catch (err) {
      console.error(err);
      setError((err as Error)?.message ?? "Failed to complete delivery. Please try again.");
    } finally {
      setIsSubmitting(false);
    }
  }, [order, hasSignature, getSignatureBlob, onComplete, onClose]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6 shadow-2xl">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Complete Delivery</h3>
            <p className="text-xs text-gray-500 dark:text-gray-400">Collect the customer's signature below before marking this order as delivered.</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="mt-6 space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <p className="text-sm font-medium text-gray-900 dark:text-gray-100" title={order.id}>{order.id}</p>
            </div>
            <div>
              <p className="text-xs uppercase text-gray-500 dark:text-gray-400">Order Status</p>
              <Badge className="mt-1 capitalize">{(order.orderStatus || "enroute").toString().replace(/-/g, " ")}</Badge>
            </div>
            <div>
              <p className="text-xs uppercase text-gray-500 dark:text-gray-400">Customer</p>
              <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{order.customerName ?? "Unknown"}</p>
            ...
            <div>
              ...
              <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">{formatCurrency(order.totalAmount)}</p>
            </div>
            <div>
              <p className="text-xs uppercase text-gray-500 dark:text-gray-400">Pickup</p>
              <p className="text-sm text-gray-700 dark:text-gray-300">{order.pickupOrder ? "Customer pickup" : formatAddress(order.pickupLocation)}</p>
            </div>
            <div>
              <p className="text-xs uppercase text-gray-500 dark:text-gray-400">Dropoff</p>
              <p className="text-sm text-gray-700 dark:text-gray-300">{order.pickupOrder ? "Customer pickup" : formatAddress(order.dropoffLocation ?? order.deliveryAddress)}</p>
            </div>
          </div>

          <div>
            <p className="text-xs uppercase text-gray-500 dark:text-gray-400 mb-2">Customer Signature</p>
            ...
            <div className="mt-2 flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
              <span>Use your finger or mouse to draw the customer's signature.</span>
              <button
                type="button"
                onClick={handleClear}
