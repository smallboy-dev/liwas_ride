...
import { useDriverStripeConnect, DriverStripeAccountStatusResponse } from "@/hooks/useDriverStripeConnect";
...
function DriverPaymentsContent() {
  const { userData, loading: authLoading, isFullyAuthorized } = useRequireRoleAndApproval(
    ["driver"],
    true
  );
  const searchParams = useSearchParams();
  const { setupStripeConnect, getAccountStatus, loading, error } = useDriverStripeConnect();
  
  const [accountStatus, setAccountStatus] = useState<DriverStripeAccountStatusResponse | null>(null);
  ...
  return (
    <DriverLayout pageTitle="Payout Setup">
      <div className="space-y-6">
        {/* Header */}
        <div className="space-y-2">
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            Set up Stripe Connect to receive payouts from your deliveries
          </p>
        </div>
                      ...
                      <div className="flex items-start">
                        ...
                        <div>
                          ...
                          <p className="mt-1 text-sm text-green-700 dark:text-green-300">
                            Your Stripe account is fully set up and ready to receive payouts. You will receive payments automatically after deliveries are completed.
                          </p>
                        </div>
                      </div>
                    </div>
                ...
                <div className="text-center py-12">
                  <div className="mb-6">
                    ...
                    <p className="text-gray-600 dark:text-gray-400 max-w-md mx-auto">
                      Connect your Stripe account to start receiving payouts from your deliveries. The setup process takes just a few minutes.
                    </p>
                  </div>
                  ...
                  <button
                    onClick={handleSetupPayments}
                    disabled={settingUp || loading}
                    className={`w-full sm:w-auto px-8 py-3 text-base font-medium text-white bg-brand-primary-600 hover:bg-brand-primary-700 rounded-lg transition-colors flex items-center justify-center space-x-2 mx-auto ${
                      settingUp || loading ? "opacity-50 cursor-not-allowed" : ""
                    }`}
                  >
                    {settingUp || loading ? (
          ...
          <div className="flex items-start">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-blue-700 dark:text-blue-300">
                ...
                you'll automatically receive payments for completed deliveries. All transactions are secure and handled by Stripe.
              </p>
            </div>
          </div>
        </div>
...
export default function DriverPaymentsPage() {
  return (
    <Suspense fallback={
      <DriverLayout pageTitle="Payments">
